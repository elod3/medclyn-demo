...
const ROOM_FRAG = /* glsl */`
  ...
  void main(){
    ...
    if (uGrid > 0.5){
      vec3 c = uBase * 0.30;
      c = mix(c, uBase * 0.95, gridLine(m, 1.0, 1.0) * 0.70);   // 1 m
      c = mix(c, vec3(0.14, 0.75, 0.80), gridLine(m, 5.0, 1.5) * 0.80);   // 5 m, cyan
...
export function createCalc(opts){
  ...
  function layout(){
    ...
    human.position.set(-L / 2 + 1.5, 0, W / 2 - 1.7);
  }
  ...
  function compute(){
    ...
    put('room-tag', 'hală ' + nf1.format(L) + ' × ' + nf1.format(W) + ' × ' + nf1.format(H) + ' m');
  }
  ...
  function readInputs(){
    dims.L = clampNum(document.getElementById('in-l').value, 2, 120, 24);
    dims.W = clampNum(document.getElementById('in-w').value, 2, 120, 12);
  ...
  const REF_DIST = REF_DIAG * 0.88 + 2.6;
  ...
  function render(dt){
    ...
    // Camera se departeaza mai incet decat creste hala (exponent < 1), ca sa se
    // VADA cresterea. Daca ar urma-o liniar, orice hala ar parea la fel de mare.
    const diag = Math.hypot(shown.L, shown.W, shown.H);
    const fit = diag * 0.88 + 2.6;
    const grow = REF_DIST * Math.pow(diag / REF_DIAG, 0.68);
    const dist = Math.max(grow, fit * 0.92);
